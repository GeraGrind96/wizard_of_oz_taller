...
Ice.loadSlice("-I ./src/ --all ./src/Display.ice")
...
class ImgData(list):
    def __init__(self, iterable=list()):
        super(ImgData, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, byte)
        super(ImgData, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, byte)
        super(ImgData, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, byte)
        super(ImgData, self).insert(index, item)

setattr(RoboCompDisplay, "ImgData", ImgData)
...
class Requires:
    def __init__(self, ice_connector):
        ...
        self.Display = self.create_proxy("DisplayProxy", RoboCompDisplay.DisplayPrx)
